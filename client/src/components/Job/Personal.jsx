...
const Personal = () => {
  ...
  const personal = [
    ...
    { tag: 'input', label: 'Present Address:', required: 'required', name: 'address', type: 'text', value: `${state.address}`, size: '30' },
    ...
    { tag: 'input', label: 'Phone#:', required: 'required', name: 'phone', type: 'text', value: `${state.phone}`, size: '15' },
  ...
  return (
    <fieldset>
      {personal.map((input) => (
        ...
        <div className='boxes' key={input.name}>
          <label className={input.required} htmlFor={input.name}>{input.label}</label>
          <input id={input.name} name={input.name} required={input.required} type={input.type} value={input.value} autoComplete='off' onChange={handleValue} size={input.size} min={input.min} max={input.max}/>
        </div>
      ))}
    </fieldset>
  );
